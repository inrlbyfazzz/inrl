...
if (conf.LANG == 'EN') vtalk_dsc = 'Starts to Inrl voice chat.', reply_eva = '*Reply to Any Voice Message!*'
...
faz.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Inrl') && conf.INRL_AI !== 'true') {        
        ...
        var finm = message.message.replace('Inrl', '').replace(' ', '')   
...
faz.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.INRL_AI == 'true' && ((!message.jid.includes('-')) || (message.jid.includes('-') && 
...
faz.addCommand({ pattern: 'vtalk$', desc: vtalk_dsc,dontAddCommandList: true, fromMe: wk }, (async (message, match) => {
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel Inrl özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'Inrl yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'Inrl yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
...
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional Inrl features. Turn your account into a ai chatbot!'
...
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ Inrl സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'Inrl കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Inrl നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Inrl പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Inrl സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

faz.addCommand({ pattern: 'chatbot ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.chatbot on / off' }, (async (message, match) => {
    var pinky_status = `${conf.INRL_AI}`
    if (match[1] == 'on') {
