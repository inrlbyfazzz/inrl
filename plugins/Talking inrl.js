...
faz.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('inrl') && conf.TALKING_INRL !== 'true') {        
...
faz.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
        if (conf.TALKING_INRL == 'true' && ((!message.jid.includes('g.us')) || (message.jid.includes('g.us') && 
...
faz.addCommand({ pattern: 'inrl ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.inrl on / off' }, (async (message, match) => {
    var inrl_status = `${conf.TALKING_INRL}`
    if (match[1] == 'on') {
        if (inrl_status == 'true') {
